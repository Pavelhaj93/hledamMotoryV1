...
export async function DELETE(req: Request, props: { params: Promise<{ motorId: string }> }) {
  ...
  try {
    ...
    const motor = await prismaDB.motorHead.findUnique({
      where: {
        id: motorId,
      },
      select: {
    ...
    await prismaDB.motorHead.delete({
      where: {
        id: motorId,
      },
    });
...
export async function PUT(req: Request, props: { params: Promise<{ motorId: string }> }) {
  ...
  try {
    ...
    const motor = await prismaDB.motorHead.update({
      where: {
        id: motorId,
      },
      data: {
    ...
    revalidatePath("/");
    revalidatePath(`/kategorie/motorove-hlavy`);
    revalidatePath("/produkt/motorova-hlava/" + motor.slug);
